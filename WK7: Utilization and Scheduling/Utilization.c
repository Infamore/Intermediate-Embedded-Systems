...
typedef struct task {
  ...
  unsigned long WCET; //the WCET of task
...
int main() {
   ...
   utilization = (tasksPeriodLCM/tasks[0].period)*tasks[0].WCET + (tasksPeriodLCM/tasks[1].period)*tasks[1].WCET + (tasksPeriodLCM/tasks[2].period)*tasks[2].WCET;
   return 0;
}
